...
class App:
    ...
    def activeCell(self):
        ...
        return self.xl.ActiveCell

    @property
    def activeChart(self):
        """返回一个 Chart 对象，该对象代表活动图表（嵌入式图表或图表工作表）。嵌入式图表在被选中或激活时被认为是活动的。当没有图表处于活动状态时，此属性返回 Nothing。"""
        return self.xl.	ActiveChart

    @property
    def activeEncryptionSession(self):
        """只读"""
        return self.xl.	ActiveEncryptionSession

    @property
    def activePrinter(self):
        """返回或设置活动打印机名称。可读/写 String 类型"""
        return self.xl.ActivePrinter

    @property
    def activeProtectedViewWindow(self):
        """返回一个 ProtectedViewWindow 对象，该对象代表活动“受保护的视图”窗口（顶部窗口）。只读。如果没有任何打开的“受保护的视图”窗口，则返回 Nothing。只读。"""
        return self.xl.ActiveProtectedViewWindow
    ...
    def activeWindow(self):
        """返回一个 Window 对象，该对象代表活动窗口（顶部窗口）。只读。如果没有打开的窗口，则返回 Nothing。"""
        return self.xl.ActiveWindow

    @property
    def activeWorkbook(self):
        """返回一个 Workbook 对象，该对象代表活动窗口（顶部窗口）中的工作簿。只读。如果没有打开的窗口，或者“信息”窗口或“剪贴板”窗口为活动窗口，则返回 Nothing。"""
        return self.xl.ActiveWorkbook

    @property
    def addIns(self):
        """返回一个 AddIns 集合，该集合代表“加载项”对话框（“开发工具”选项卡上的“加载项”命令）中列出的所有加载项。只读。"""
        return self.xl.AddIns

    @property
    def alertBeforeOverwriting(self):
        """	如果 ET 在执行拖放编辑操作过程中，在覆盖非空单元格之前显示一条消息，则该属性值为 True。Boolean 类型，可读写。"""
        return self.xl.AlertBeforeOverwriting

    @property
    def altStartupPath(self):
        """返回或设置替换启动文件夹的名称。String 类型，可读写。"""
        return self.xl.AltStartupPath

    @property
    def alwaysUseClearType(self):
        """	返回或设置一个 Boolean 值，该值代表是否使用 ClearType 显示菜单、功能区和对话框文本中的字体。可读写 Boolean。"""
        return self.xl.AlwaysUseClearType
    ...
    def application(self):
        ...
        return self.xl.Application

    @property
    def arbitraryXMLSupportAvailable(self):
        """返回一个 Boolean 类型的值，该值表示 ET 中的 XML 功能是否可用。只读。"""
        return self.xl.ArbitraryXMLSupportAvailable

    @property
    def askToUpdateLinks(self):
        """	如果 ET 打开带有链接的文件时询问用户是否更新链接，则该值为 True。如果 ET 自动更新链接并且不显示对话框，则该值为 False。Boolean 类型，可读写。"""
        return self.xl.AskToUpdateLinks

    @property
    def automationSecurity(self):
        """	返回或设置一个 MsoAutomationSecurity 常量，该常量表示在用编程方式打开文件时，ET 所使用的安全模式。可读写。"""
        return self.xl.AutomationSecurity

    @property
    def autoRecover(self):
        """返回一个 AutoRecover 对象，用于按设定的时间间隔备份所有文件格式。
        有效的时间间隔为从 1 到 120 的所有数值。
        在本示例中，Time 属性与 AutoRecover 属性结合使用，可将 ET 在保存另一个副本之前的等待时间间隔设置为 5 分钟。
        ...
        return self.xl.AutoRecover
    @property
    def build(self):
        """返回 ET 内部版本号。Long 类型，只读。"""
        return self.xl.Build


    def calculateBeforeSave(self):
        """如果在将工作簿保存到磁盘之前计算工作簿（如果 Calculation 属性设置为 xlManual），则该属性值为 True。如果更改 Calculation 属性则保留该属性。Boolean 类型，可读写。"""
        return self.xl.CalculateBeforeSave
    ...
    def calculationInterruptKey(self):
        ...
        return self.xl.CalculationInterruptKey

    @calculationInterruptKey.setter
    def calculationInterruptKey(self,value):
        """设置或返回一个 XlCalculationInterruptKey 常量，该常量指定在执行计算时可中断 ET 的键。可读写。"""
        self.xl.CalculationInterruptKey=value

    @property
    def calculationState(self):
        """返回一个 XlCalculationState 常量，该常量针对 ET 中正在执行的任意计算指示应用程序的计算状态。只读。"""
        return self.xl.CalculationState

    @property
    def calculationVersion(self):
        """返回一个数字，其最右侧两位表示计算引擎的次要版本号，而左侧的其他位表示 WPS Office Spreadsheet 组件的主要版本号。Long 型，只读。"""
        return self.xl.CalculationVersion

    @property
    def caption(self):
        """返回或设置一个 String 值，它代表出现在 ET 主窗口标题栏中显示的名称。"""
        return self.xl.Caption

    @property
    def cellDragAndDrop(self):
        """	如果启用单元格拖放功能，则该属性值为 True。Boolean 类型，可读写。"""
        return self.xl.CellDragAndDrop

    @cellDragAndDrop.setter
    def cellDragAndDrop(self,attributes=False):
        """	如果启用单元格拖放功能，则该属性值为 True。Boolean 类型，可读写。"""
        self.xl.CellDragAndDrop=attributes

    @property
    def cells(self):
        """	返回一个 Range 对象，该对象代表活动工作表中的所有单元格。如果活动文档不是工作表，则此属性无效。"""
        return self.xl.Cells

    @property
    def charts(self):
        # 暂时运行不了
        """返回或设置替换启动文件夹的名称。String 类型，可读写。"""
        return self.xl.Charts

    @property
    def clipboardFormats(self):
        """以数字数组的形式返回剪贴板中当前的格式。可用数组中每个元素与“备注”部分所述的适当常量相比较，以判断剪贴板中是否包含特定的格式。Variant 类型，只读。"""
        return self.xl.ClipboardFormats

    @property
    def clusterConnector(self):
        """返回或设置用于运行 XLL 加载项中用户定义的函数的高性能计算 (HPC) 群集连接器的名称。可读写。"""
        return self.xl.ClusterConnector

    @clusterConnector.setter
    def clusterConnector(self,value):
        """返回或设置用于运行 XLL 加载项中用户定义的函数的高性能计算 (HPC) 群集连接器的名称。可读写。"""
        self.xl.ClusterConnector=value

    @property
    def columns(self):
        # 暂时运行不了
        """返回一个 Range 对象，它代表活动工作表中的所有列。如果活动文档不是工作表，则 Columns 属性无效。"""
        return self.xl.Columns

    @property
    def comAddIns(self):
        """返回 ET 的 COMAddIns 集合，该集合代表当前所安装的 COM 加载项。只读。"""
        return self.xl.COMAddIns

    @property
    def commandBars(self):
        """返回一个代表 ET 命令栏的 CommandBars 对象。只读。"""
        return self.xl.CommandBars

    @property
    def commandUnderlines(self):
        """	返回或设置 ET for the Macintosh 中命令加下划线的状态。可以是 XlCommandUnderlines 常量之一。Long 类型，可读写。"""
        return self.xl.CommandUnderlines

    @commandUnderlines.setter
    def commandUnderlines(self,value='xlCommandUnderlinesOn'):
        """	返回或设置 ET for the Macintosh 中命令加下划线的状态。可以是 XlCommandUnderlines 常量之一。Long 类型，可读写。
        在 ET for Windows 中，该属性总是返回 xlCommandUnderlinesOn，该属性只能设置为 xlCommandUnderlinesOn，不允许设置为其他任何值。"""
        self.xl.CommandUnderlines=value

    @property
    def constrainNumeric(self):
        """如果手写识别只能识别数字和标点符号，则该属性值为 True。Boolean 类型，可读写。"""
        return self.xl.ConstrainNumeric

    @constrainNumeric.setter
    def constrainNumeric(self,attributes=False):
        """如果手写识别只能识别数字和标点符号，则该属性值为 True。Boolean 类型，可读写。"""
        self.xl.ConstrainNumeric=attributes

    @property
    def controlCharacters(self):
        """如果 ET 显示从右到左语言的控制字符，则该属性值为 True。Boolean 类型，可读写。"""
        return self.xl.ControlCharacters

    @controlCharacters.setter
    def controlCharacters(self,attributes=False):
        """如果 ET 显示从右到左语言的控制字符，则该属性值为 True。Boolean 类型，可读写。"""
        self.xl.ControlCharacters=attributes

    @property
    def copyObjectsWithCells(self):
        """如果对象与单元格一同剪切、复制、提取和排序，则该属性值为 True。Boolean 类型，可读写。"""
        return self.xl.ControlCharacters

    @copyObjectsWithCells.setter
    def copyObjectsWithCells(self, attributes=True):
        """如果对象与单元格一同剪切、复制、提取和排序，则该属性值为 True。Boolean 类型，可读写。"""
        self.xl.CopyObjectsWithCells = attributes

    @property
    def creator(self):
        """返回一个 32 位整数，该整数指示创建对象的应用程序。只读 Long 类型。"""
        return self.xl.Creator
    ...
    def cursor(self, value=-4143):
        ...
        xlWait	2	沙漏型指针。 """
        self.xl.Cursor = value

    @property
    def cursorMovement(self):
        """返回或设置一个值，此值用于表明使用的是可见光标还是逻辑光标。可为以下常量之一：xlVisualCursor 或 xlLogicalCursor。Long 类型，可读写。"""
        return self.xl.ControlCharacters

    @cursorMovement.setter
    def cursorMovement(self, value:int):
        """返回或设置一个值，此值用于表明使用的是可见光标还是逻辑光标。可为以下常量之一：xlVisualCursor 或 xlLogicalCursor。Long 类型，可读写。"""
        self.xl.CursorMovement = value

    @property
    def customListCount(self):
        """返回已定义的自定义序列的数目（包括内置序列）。Long 类型，只读。"""
        return self.xl.CustomListCount

    @property
    def cutCopyMode(self):
        """返回或设置剪切或复制模式的状态。可为 True、False 或如下表所示的一个 XLCutCopyMode 常量。Long 类型，可读写。"""
        return self.xl.CutCopyMode

    @cutCopyMode.setter
    def cutCopyMode(self, value:int):
        """返回或设置剪切或复制模式的状态。可为 True、False 或如下表所示的一个 XLCutCopyMode 常量。Long 类型，可读写。"""
        self.xl.CutCopyMode = value

    @property
    def dataEntryMode(self):
        """返回或设置“数据输入”模式，如下表所示。处于“数据输入”模式时，仅可在当前选定区域的未锁定单元格中输入数据。Long 类型，可读写。"""
        return self.xl.DataEntryMode

    @dataEntryMode.setter
    def dataEntryMode(self, value:int):
        """返回或设置“数据输入”模式，如下表所示。处于“数据输入”模式时，仅可在当前选定区域的未锁定单元格中输入数据。Long 类型，可读写。"""
        self.xl.DataEntryMode = value

    @property
    def dDEAppReturnCode(self):
        """	返回带应用程序说明符的 DDE 返回代码，该代码包含在 ET 接收到的最后一个 DDE 确认消息中。Long 类型，只读。。"""
        return self.xl.DDEAppReturnCode

    @property
    def decimalSeparator(self):
        """	设置或返回用作小数分隔符的字符。String 类型，可读写。"""
        return self.xl.DecimalSeparator

    @decimalSeparator.setter
    def decimalSeparator(self, value:str):
        """	设置或返回用作小数分隔符的字符。String 类型，可读写。"""
        self.xl.DecimalSeparator = value

    @property
    def defaultSaveFormat(self):
        """返回或设置保存文件的默认格式。要获取有效常量的列表，请参阅 FileFormat 属性。Long 类型，可读写。"""
        return self.xl.DefaultSaveFormat

    @defaultSaveFormat.setter
    def defaultSaveFormat(self, value:int):
        """返回或设置保存文件的默认格式。要获取有效常量的列表，请参阅 FileFormat 属性。Long 类型，可读写。"""
        self.xl.DefaultSaveFormat = value
